            ...
            <div class="col-md-12 col-sm-12 ">
              <div class="x_panel">
                ...
                <div class="x_content">
                  <br />
                  @if(isset($user['id']))
                    <form method="POST" action="{{route($common.'.update',$user['id'])}}" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data">
                    @method('PUT')
                  @else
                  ...
                  @csrf
                      ...
                      <div class="col-md-6 col-sm-6 ">
                        <select class="form-control" name="batch_id">
                          @foreach($batches as $batch)
                            <option value="{{$batch['id']}}" @if(isset($user['batch_id']) && $user['batch_id'] == $batch['id']) selected @endif>{{$batch['name']}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="item form-group">
                      ...
                      <div class="col-md-6 col-sm-6 ">
                        <input type="email" name="email" class="form-control" value="{{$user['email'] ?? ''}}" @if(isset($user['id'])) readonly="" @endif>
                      </div>
                    </div>
